...
public class StatementInterpreter implements StatementVisitor<Value> {
    ...
    public Value visitIfStatement(IfStatement node) {
        for (Map.Entry<Expression, BlockStatement> entry : node.ifElseBranch().entrySet()) {
            Value conditionValue = entry.getKey().accept(this.context.getInterpreter());
            if (conditionValue instanceof BooleanValue boolVal) {
                if (boolVal.value()) {
                    this.context.scope(() -> {
                        entry.getValue().accept(this.context.getInterpreter());
                    });
                    return null;
                }
            } else {
        ...
        if (iterableValue instanceof ListValue listVal) {
            this.context.loop(() -> {
                for (Value item : listVal.value()) {
                    this.context.getScope().setVariable(node.variable().getValue(), item);
