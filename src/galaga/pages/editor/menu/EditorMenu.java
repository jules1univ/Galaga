...
public class EditorMenu extends Page<GalagaPage> {
    ...
    public void update(float dt) {
        ...
        if (Galaga.getContext().getInput().isKeyPressed(
                Galaga.getContext().getState().keyboard.getKey(Config.KEYBOARD_MENU_NAVIGATE_LEFT)
                        .orElse(KeyEvent.VK_LEFT))) {
            ...
            this.selectSound.play(2.f);
                ...
                Galaga.getContext().getState().keyboard.getKey(Config.KEYBOARD_MENU_NAVIGATE_RIGHT)
                        .orElse(KeyEvent.VK_RIGHT))) {
            this.selectSound.play(2.f);
            if (this.option == EditorMenuOption.EDITORS) {
                this.etidorMode.next();
            }
        } else if (Galaga.getContext().getInput().isKeyPressed(
                Galaga.getContext().getState().keyboard.getKey(Config.KEYBOARD_MENU_NAVIGATE_UP)
                        .orElse(KeyEvent.VK_UP))) {
            this.selectSound.play(2.f);
            switch (this.option) {
                case EDITORS -> {
                ...
                Galaga.getContext().getState().keyboard
                        .getKeys(Config.KEYBOARD_MENU_NAVIGATE_DOWN, Config.KEYBOARD_MENU_NAVIGATE)
                        .orElse(List.of(KeyEvent.VK_DOWN, KeyEvent.VK_TAB)))) {
            this.selectSound.play(2.f);
            switch (this.option) {
                case EDITORS -> {
                    this.option = EditorMenuOption.SETTINGS;
            ...
            this.updateMenuSelect();
                ...
                Galaga.getContext().getState().keyboard.getKey(Config.KEYBOARD_MENU_CONFIRM)
                        .orElse(KeyEvent.VK_ENTER))) {
            this.selectSound.play(2.f);
            switch (this.option) {
                case EDITORS -> {
